...
plot(WAV,stdData(1,:))
xlabel('Wavelength')
ylabel('Standard deviation for some concentration')

figure
plot([1:26],stdData(:,5),'x')
xlabel('Sample')
ylabel('Standard deviation for some wavelength')
%%

for i=1:26
 %DATA1(i,:)=DATA(5*(i-1)+1,:)./stdData(i,:);         % select 1st of each blocks     
 DATA1(i,:)=mean(DATA([5*i-4:5*i],:))./stdData(i,:); % select mean of each block 
